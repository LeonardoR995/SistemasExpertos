...
# Función para mostrar la ventana de selección de fórmulas
def mostrar_ventana_seleccion():
    ventana_seleccion = tk.Toplevel()
    ventana_seleccion.title("Seleccionar Fórmula")

    # Crear un Listbox para mostrar las fórmulas
    listbox_formulas = tk.Listbox(ventana_seleccion, height=10, width=50, font=("Helvetica", 10))
    for i, formula in enumerate(formulas):
        listbox_formulas.insert(tk.END, f"Fórmula {i + 1}: {formula}")
    listbox_formulas.pack(pady=10)

    # Botón para mostrar la fórmula seleccionada
    def mostrar_fórmula_seleccionada():
        seleccion = listbox_formulas.curselection()
        if seleccion:
            index = seleccion[0]
            formula = formulas[index]
            mostrar_detalles_formula(formula)

    boton_mostrar = tk.Button(ventana_seleccion, text="Mostrar Fórmula", command=mostrar_fórmula_seleccionada)
    boton_mostrar.pack(pady=5)

# Función para mostrar los detalles de la fórmula seleccionada
def mostrar_detalles_formula(formula):
    ventana_detalles = tk.Toplevel()
    ventana_detalles.title("Detalles de la Fórmula")

    # Mostrar la fórmula seleccionada
    etiqueta_formula = tk.Label(ventana_detalles, text=f"Fórmula: {formula}", font=("Helvetica", 12, "bold"))
    etiqueta_formula.pack(pady=10)

    # Agregar un botón para cerrar la ventana
    boton_cerrar = tk.Button(ventana_detalles, text="Cerrar", command=ventana_detalles.destroy)
...
def mostrar_fórmulas():
    ventana_seleccion = tk.Toplevel()
    ventana_seleccion.title("Seleccionar Fórmula")

    if not formulas:
        messagebox.showwarning("Advertencia", "No hay fórmulas disponibles.")
        return
    ...
    # Función para mostrar la fórmula seleccionada
    def mostrar_fórmula_seleccionada():
        seleccion = listbox_formulas.curselection()
        if seleccion:
            index = seleccion[0]
...
# Función para procesar las proposiciones y generar las fórmulas
# Función para procesar las proposiciones y generar las fórmulas
def procesar_proposiciones():
    # Declaramos todas las variables globales al inicio de la función
    global operadores_list, proposiciones_simples_list, formulas, proposiciones_map

    # Ahora inicializamos las variables globales
    operadores_list = []  # Lista de operadores
    proposiciones_simples_list = []  # Lista de proposiciones simplificadas
    formulas = []  # Lista de fórmulas completas
    proposiciones_map = {}  # Diccionario de mapeo de proposiciones
    ...
    # Asegúrate de que la función de selección de fórmulas se llame después de que se procesen las proposiciones
    if operadores_list and proposiciones_simples_list:
        mostrar_ventana_seleccion()  # Esta función muestra la ventana de selección de fórmulas
...
# Botón para mostrar las fórmulas generadas
boton_mostrar_formulas = tk.Button(ventana_principal, text="Mostrar Fórmulas", font=("Helvetica", 10), bg="#28a745", fg="white", command=mostrar_fórmulas)
boton_mostrar_formulas.pack(pady=10)
